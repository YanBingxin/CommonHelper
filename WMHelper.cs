...
namespace WMHelper
{
    ...
    public class WmiShareFunction
    {
        ...
        /// <param name="DirectoryName">要建立的目录名(必须包含根目录)</param>
        /// <returns>0表示建立成功，-1表示建立失败</returns>
        public static int CreateRemoteDirectory(ManagementScope scope, string DirectoryName)
        {
            try
            {
                ...
                inParams["CommandLine"] = @"cmd /C Md " + DirectoryName;
                inParams["CurrentDirectory"] = Path.GetPathRoot(DirectoryName);

                ManagementBaseObject outParams = null;
                outParams = processBatch.InvokeMethod("Create", inParams, null);

                //Wmilog.WriteToLog("Excute CreateRemoteDirectory:" + outParams.ToString());v////Wmilog////Wmilog
            }
            catch (Exception ex)
            {
                     ...
                     )
        {
            ...
            try
            {
                ...
                if (scope.IsConnected)
                {
                    ...
                    RemoveShareNetConnect(Server, ShareName);
        ...
        /// <param name="Username">远程登录用户</param>
        /// <param name="Password">远程登录密码</param>
        public static void CreateShareNetConnect(string ServerIP, string ShareName, string Username, string Password)
        {
            Process process = new Process();
            string arg =string.Format("use \\\\{0}\\{1} \"{2}\" /user:\"{3}\"",ServerIP,ShareName,Username,Password);
